      ...
      <div id="modal1" class="modal small-modal-1">
         <div class="modal-content">
             ...
             <form  method="post"  id="category_form">
                 ...
                 <div class="row m0">
                    <h6 class="m-title col">Add Events</h6>

                     <div class="input-field col s12">
                       <input type="text" id="title" name="title" placeholder="Enter Title" class="validate" autofocus required >
                       <label for="title" class="active">Title <span class="red-text">*</span></label>
                       <input type="hidden" id="ctid" name="ctid">
                     </div>

                     <div class="input-field col s12">
                       <input type="url" id="url" name="url" placeholder="Enter url" class="validate"  required >
                       <label for="url" class="active">Url <span class="red-text">*</span></label>
                     </div>

                     <div class="input-field col s12">
                       <input type="date" id="date" value="" min="<?php echo date('Y-m-d') ?>" name="date" placeholder="." class=""  required >
                       <label for="date" class="active">Date <span class="red-text">*</span></label>
                     </div>
      ...
      <script>
        $(document).ready( function () {
            ...
            var dataTable = $('#dynamic').DataTable({
                  ...
                  'order' : [],
                  'ajax':{
                     'url' : "<?php echo base_url(). 'events/getData' ?>",
                     'type' :'POST'
                  },
                  'columnDefs':[
                     {
            ...
            $(document).on('click', '.update-btn', function(){
               ...
               $('.m-title').text('Edit Event');
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url(); ?>events/single_data",
                    data: {id : id},
                    dataType: "json",
            ...
            $(document).on('submit', '#category_form', function(event){  
                ...
                if(id != ''){
                    ...
                    $.ajax({  
                      url:"<?php echo base_url() . 'events/update_events'?>",  
                      method:'POST',  
